...
y = []
while flag:
    y = [[3.8 + 6.4 * x_nat[i][1] + 4.8 * x_nat[i][2] + 6.9 * x_nat[i][3] + 9 * x_nat[i][1] * x_nat[i][1] +
          0.2 * x_nat[i][2] * x_nat[i][2] + 5.2 * x_nat[i][3] * x_nat[i][3] + 2.6 * x_nat[i][1] * x_nat[i][2] +
          1 * x_nat[i][1] * x_nat[i][3] + 0.6 * x_nat[i][2] * x_nat[i][3] + 1.8 * x_nat[i][1] * x_nat[i][2] * x_nat[i][3]
...
for i in range(len(coef)):
    if coef[i] == 1:
        d += 1
f4 = 14 - d
S_ad = (m / (f4)) * (pow((yQ[0][0] - ySr[0]), 2) + pow((yQ[1][0] - ySr[1]), 2) + pow((yQ[2][0] - ySr[2]), 2) + pow(
    (yQ[3][0] - ySr[3]), 2)
...
k0, k1, k2, k3, k4, k5, k6, k7 = 0, 0, 0, 0, 0, 0, 0, 0
for i in range(14):
    k0 += ySr[i]
    k1 += ySr[i] * x_nat[i][1]
    k2 += ySr[i] * x_nat[i][2]
...
print("y = ", round(b5[0], 4), "+", round(b5[1], 4), " * x1 +", round(b5[2], 4), " * x2 +",
      ...
      "* x3^2")
Dbeta_1 = math.sqrt(sum(D)/14/(15*m))
